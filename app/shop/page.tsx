    ...
    <div className="pt-14 min-h-screen bg-background flex flex-col">
      <div className="px-4 sm:px-6 max-w-[1400px] mx-auto w-full flex-1">
        ...
        <motion.div 
          ...
          className="flex gap-4 sm:gap-6 mb-6 sm:mb-8 overflow-x-auto no-scrollbar sm:justify-end border-b border-border/40 pb-2 sm:border-none"
        >
          {categories.map((cat) => (
            <button
              key={cat}
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-baseline py-3 sm:py-4 gap-1">
                  <Link href={`/product/${product.id}`}>
                    <span className="fossil-label text-foreground text-[10px] sm:text-[12px] hover:opacity-60 transition-opacity">
                      {product.name}
                    </span>
                  </Link>
