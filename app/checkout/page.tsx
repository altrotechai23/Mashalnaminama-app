...
export default function CheckoutPage() {
  ...
  if (cart.items.length === 0) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center pt-20 gap-4">
        ...
        <p className="text-zinc-500 uppercase text-[10px] tracking-[0.3em]">
          Your cart is empty
        </p>
      </div>
    );
  ...
  setLoading(true);
  try {
    const { orderId } = await createOrder(cart.items, totalPrice);

    const res = await fetch("/api/paypal/create-order", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ totalAmount: totalPrice, orderId }),
    });
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded-[2rem] p-8 space-y-8 shadow-2xl">
          ...
          <div className="pt-4 flex justify-between items-end">
            <div>
              <p className="text-[10px] uppercase font-black text-zinc-600 tracking-widest mb-1">Total</p>
              <p className="text-4xl font-mono tracking-tighter text-white">R{totalPrice.toFixed(2)}</p>
            </div>
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={18} /> : <><Lock size={16} /> Pay via PayPal</>}
          </button>
        </div>
      </div>
    </div>
